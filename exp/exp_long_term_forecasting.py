...
from utils.tools import EarlyStopping, adjust_learning_rate, visual,timeit
...
class Exp_Long_Term_Forecast(Exp_Basic):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[
                            0 if self.args.output_attention else None]
                else:
                    outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[
                        0 if self.args.output_attention else None]
    ...
    def test(self, setting, load_model=True):
        ...
        f.write(setting + "  \n")
        f.write('mae:{:.2f}, mape:{:.5f}'.format(mse, mae))
        f.write('\n')
        f.write('\n')
        f.close()
